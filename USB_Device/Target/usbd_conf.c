...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev) {
  ...
  /* USER CODE BEGIN EndPointConfiguration */

  /* EP0 — Control (unchanged) */
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x00U, PCD_SNG_BUF,
                      0x040U);
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x80U, PCD_SNG_BUF,
                      0x080U);

  /* EP1 IN  — IF0 CC Sniffer -> Host (libsigrok) */
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x81U, PCD_SNG_BUF,
                      0x0C0U);

  /* EP1 OUT (0x01) — Future: NOT configured, NOT opened */
  /* HAL_PCDEx_PMAConfig(..., 0x01U, PCD_SNG_BUF, 0x???U);  <-- add when ready
   */

  /* EP2 IN  — IF1 Analog Parameters + Command Responses -> Host */
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x82U, PCD_SNG_BUF,
                      0x140U);

  /* EP2 OUT — IF1 Commands <- Host  (reuses former EP1 OUT PMA slot) */
  HAL_PCDEx_PMAConfig((PCD_HandleTypeDef *)pdev->pData, 0x02U, PCD_SNG_BUF,
                      0x100U);

  /* USER CODE END EndPointConfiguration */
  return USBD_OK;
}
...
void *USBD_static_malloc(uint32_t size) {
  static uint32_t mem[(sizeof(USBD_THUNDER_HandleTypeDef) / 4) + 1];
  return mem;
}
